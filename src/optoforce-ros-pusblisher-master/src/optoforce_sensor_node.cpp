...
int main(int argc, char **argv)
{
    ...
    if (ports.getLastSize()>0)
    {
        ...
        if (daq.getVersion()!=_95 && daq.getVersion() != _64) // It is a 3D sensor
        {
            ...
            std::cout<<"optoforce_sensor_node online"<<std::endl;
            //Support should be added later

            ros::init(argc, argv, "ForceSensorPublisher");
            ros::NodeHandle n("~");

            ros::Publisher optoforce_data = n.advertise<geometry_msgs::Vector3>("optoforce_3D",100);
            ros::Rate rate(200);

            while(ros::ok()) {
            //Declares the message to be sent
                geometry_msgs::Vector3 msg;
                size=daq.read(pack3D,false);
