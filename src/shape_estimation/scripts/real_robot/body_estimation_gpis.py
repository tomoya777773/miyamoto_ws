...
def body_error(true_surface, model):
    center_po    = np.array([0.373873986156, 0.215548561551, 0.162]) 
...
if __name__=="__main__":
    ...
    ############################################# object surface ###################################################################
    X_body    = np.load('../data/body/{}/gpis/value/body.npy'.format(object))
    body_data = np.load('../data/body/{}/gpis/value/true_body.npy'.format(object))
    # head_data = np.load('../data/body/{}/gpis/value/true_head.npy'.format(object))
    print body_data[:,2].min()
    X_body[:,2] -= 0.035
    body_data[:, 2] += 0.012 # low -> 0.015
    # head_data[:, 2] += 0.015 # low -> 0.015
    ...
    # np.save('../data/body/{}/gpis/value/body'.format(object), X_body)
    # surface_data = ur.target_points
    # np.save('../data/body/{}/gpis/value/true_surface'.format(object), surface_data)

    # true body
    # surface_data = ur.target_points
    # np.save('../data/body/{}/gpis/value/true_body'.format(object), surface_data)
    # true head
    # surface_data = ur.target_points
    # np.save('../data/body/{}/gpis/value/true_head'.format(object), surface_data)
    ...
    while True:
        ...
        # if save_movie:
        #     plt.savefig('../data/body/{}/gpis/movie/step{}.png'.format(object, movie_num))
        #     movie_num += 1
        # plt.draw()
        # plt.pause(0.001)
        # plt.clf()
    ...
    if save_data:
        np.save('../data/body/{}/gpis/value/rmse'.format(object), rmse_list)
        np.save('../data/body/{}/gpis/value/var'.format(object), var_list)
