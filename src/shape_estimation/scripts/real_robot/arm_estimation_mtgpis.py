...
if __name__=="__main__":
    ...
    X_body = np.load('../data/arm/{}/mtgpis/value/arm.npy'.format(object))
    surface_data = np.load('../data/arm/{}/mtgpis/value/true_surface.npy'.format(object))
    # surface_data[:, 2] += 0.01
    # データを前もって作り保存する、真の形状
    # X_body = ur.target_points1
    # np.save('../data/arm/{}/mtgpis/value/arm'.format(object), X_body)
    # surface_data = ur.target_points
    # np.save('../data/arm/{}/mtgpis/value/true_surface'.format(object), surface_data)
    ...
    while True:
        ...
        if save_movie:
            plt.savefig('../data/arm/{}/mtgpis/movie/step{}.png'.format(object, movie_num))
            movie_num += 1
        plt.draw()
        plt.pause(0.001)
        plt.clf()
    ...
    for i in range(150):
        ...
        if i == 0:
            ...
            # task_kernel_params = torch.tensor([[0.7650, 0.9253],[0.2700, -1.0083]]) #low arm
            task_kernel_params =torch.tensor([[-0.1025, -0.1739],[0.2700, -1.5518]]) #high arm
    ...
    if save_data:
        np.save('../data/arm/{}/mtgpis/value/rmse'.format(object), rmse_list)
        np.save('../data/arm/{}/mtgpis/value/var'.format(object), var_list)
        np.save('../data/arm/{}/mtgpis/value/simirality'.format(object), simirality_list)
